...
class ChunkIn(BaseModel):
    ...
    metadata: Dict[str, Any] = Field(
        ...
        description="Optional arbitrary metadata (source, page, date, etc.)"
    )
...
class ChunkOut(ChunkIn):
    ...
    score: Optional[float] = Field(
        None,
        description="Relevance/distance score (lower = more similar in most vector DBs)",
        ge=0.0,
        le=1.0,  # adjust range depending on your distance metric
    )
...
class QueryRequest(BaseModel):
    ...
    query: str = Field(..., description="The user's natural language question/query")
    top_k: int = Field(
        default=5,
        ge=1,
        le=50,
...
class QueryResponse(BaseModel):
    ...
    # Optional: you can add these later when needed
